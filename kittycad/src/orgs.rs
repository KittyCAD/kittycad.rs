...
use crate::Client;
#[derive(Clone, Debug)]
pub struct ListDatasetConversionsParams {
    ...
    pub q: Option<String>,
    pub sort_by: Option<crate::types::ConversionSortMode>,
}
...
#[derive(Clone, Debug)]
pub struct SearchDatasetConversionsParams {
    pub filter: Option<String>,
    pub id: uuid::Uuid,
    pub limit: Option<u32>,
    pub page_token: Option<String>,
...
impl Orgs {
    ...
    pub fn list_datasets_stream<'a>(
        ...
        let stream = self
            .list_datasets(limit, lookup_enabled, None, sort_by)
            .map_ok(move |result| {
    ...
    #[doc = "Bulk-download KCL outputs for successful dataset conversions.\n\n**Parameters:**\n\n- \
             `id: uuid::Uuid`: The identifier. (required)\n\n```rust,no_run\nuse \
             std::str::FromStr;\nasync fn example_orgs_download_dataset_successful_kcl_bulk() -> \
             anyhow::Result<()> {\n    let client = kittycad::Client::new_from_env();\n    \
             client\n        .orgs()\n        \
             .download_dataset_successful_kcl_bulk(uuid::Uuid::from_str(\n            \
             \"d9797f8d-9ad6-4e08-90d7-2ec17e13471c\",\n        )?)\n        .await?;\n    \
             Ok(())\n}\n```"]
    #[tracing::instrument]
    pub async fn download_dataset_successful_kcl_bulk<'a>(
        &'a self,
        id: uuid::Uuid,
    ) -> Result<(), crate::types::error::Error> {
        let mut req = self.client.client.request(
            ...
            format!(
                "{}/{}",
                self.client.base_url,
                "org/datasets/{id}/bulk-download/kcl".replace("{id}", &format!("{}", id))
            ),
        );
        req = req.bearer_auth(&self.client.token);
        let resp = req.send().await?;
    ...
    > + Unpin
        ...
        if let Some(p) = phase.as_ref() {
            pagination_query_params.push(("phase", p.clone()));
        }
