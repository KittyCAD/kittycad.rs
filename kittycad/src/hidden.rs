...
impl Hidden {
    ...
    #[doc = "Create an email verification request for a user.\n\n```rust,no_run\nuse std::str::FromStr;\nasync fn example_hidden_listen_auth_email() -> anyhow::Result<()> {\n    let client = kittycad::Client::new_from_env();\n    let result : crate :: types :: VerificationToken = client . hidden () . listen_auth_email (& crate :: types :: EmailAuthenticationForm { callback_url : Some (url :: Url :: from_str (\"https://112.0.0.31/2\") ?) , email : \"143.126.86.111.225.20.71.126.40.141.44@244.10.118.231.228.245.103.58.191.59.250.31.41\" }) . await ? ;\n    println!(\"{:?}\", result);\n    Ok(())\n}\n```"]
    ...
    #[doc = "Listen for callbacks for email verification for users.\n\n```rust,no_run\nuse std::str::FromStr;\nasync fn example_hidden_listen_auth_email_callback() -> anyhow::Result<()> {\n    let client = kittycad::Client::new_from_env();\n    client . hidden () . listen_auth_email_callback (Some (url :: Url :: from_str (\"https://172.50.172.176.185.62.28.191.123/5\") ?) , \"234.240.188.153.229.17.40.165.14.25.212@253.10.152.186.25.235.155.215.200.171.81.145.151\" , \"pytcj\" ,) . await ? ;\n    Ok(())\n}\n```"]
