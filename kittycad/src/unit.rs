...
impl Unit {
    ...
    #[doc = "Convert time units.\n\nConvert a time unit value to another time unit value. This is \
             a nice endpoint to use for helper functions.\n\n**Parameters:**\n\n- `output_format: \
             crate::types::UnitTimeFormat`: The output format of the unit. (required)\n- \
             `src_format: crate::types::UnitTimeFormat`: The source format of the unit. \
             (required)\n- `value: f64`: The initial value. (required)\n\n```rust,no_run\nasync fn \
             example_unit_get_time_conversion() -> anyhow::Result<()> {\n    let client = \
             kittycad::Client::new_from_env();\n    let result: \
             kittycad::types::UnitTimeConversion = client\n        .unit()\n        \
