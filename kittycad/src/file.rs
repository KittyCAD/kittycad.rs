...
impl File {
    ...
    #[doc = "Get CAD file density.\n\nWe assume any file given to us has one consistent unit \
             ...
             assumes if you are giving a material mass in a specific mass units, we return a \
             density in mass unit per cubic measure unit.\nIn the future, we will use the units \
             inside the file if they are given and do any conversions if necessary for the \
             calculation. But currently, that is not supported.\nGet the density of an object in a \
             CAD file. If the file is larger than 25MB, it will be performed asynchronously.\nIf \
             the operation is performed asynchronously, the `id` of the operation will be \
             returned. You can use the `id` returned from the request to get status information \
             about the async operation from the `/async/operations/{id}` \
             endpoint.\n\n**Parameters:**\n\n- `material_mass: f64`: The material mass. \
             (required)\n- `material_mass_unit: Option<crate::types::UnitMass>`: The unit of the \
             material mass.\n- `output_unit: Option<crate::types::UnitDensity>`: The output unit \
             for the density.\n- `src_format: crate::types::FileImportFormat`: The format of the \
    ...
    #[doc = "Get CAD file volume.\n\nWe assume any file given to us has one consistent unit \
             throughout. We also assume the file is at the proper scale.\nCurrently, this endpoint \
             returns the cubic measure units.\nIn the future, we will use the units inside the \
             file if they are given and do any conversions if necessary for the calculation. But \
