...
#[doc = "Tracks state of `OrgDataset` file conversions."]
#[derive(
    serde :: Serialize, serde :: Deserialize, PartialEq, Debug, Clone, schemars :: JsonSchema,
)]
pub struct OrgDatasetFileConversion {
    ...
    pub dataset_id: uuid::Uuid,
    #[doc = "File's ETag from dataset bucket, for detecting whether a file needs to be converted \
             again can be reused when creating new custom ML models."]
    ...
    pub id: uuid::Uuid,
    #[doc = "Tracks which version the file was processed with. If conversion failed due to an \
...
impl std::fmt::Display for OrgDatasetFileConversion {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        write!(
            f,
            "{}",
...
#[cfg(feature = "tabled")]
impl tabled::Tabled for OrgDatasetFileConversion {
    const LENGTH: usize = 13;
    fn fields(&self) -> Vec<std::borrow::Cow<'static, str>> {
        vec![
            if let Some(completed_at) = &self.completed_at {
...
impl tabled::Tabled for OrgDatasetFileConversionDetails {
    ...
    fn headers() -> Vec<std::borrow::Cow<'static, str>> {
        vec!["conversion".into(), "output".into()]
    }
}
