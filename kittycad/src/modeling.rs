...
impl Modeling {
    ...
    #[doc = "Open a websocket which accepts modeling commands.\n\nPass those commands to the \
             ...
             fn example_modeling_commands_ws() -> anyhow::Result<()> {\n    let client = \
             kittycad::Client::new_from_env();\n    client.modeling().commands_ws().await?;\n    \
             Ok(())\n}\n```"]
    #[tracing::instrument]
    pub async fn commands_ws<'a>(&'a self) -> Result<(), crate::types::error::Error> {
        let mut req = self.client.client.request(
            http::Method::GET,
